  ...
  {  
    ...
    if(Taster_In)
    {
      switch (Taster_In) 
      {
          ...
          case '7':
              lvl = 4;
              LED_speed = LED_speed - (30*(lvl-3));
              delay_LEDs = delay_LEDs - (10*(lvl-3));
            break;
          case '8':
              lvl = 5;
              LED_speed = LED_speed - (30*(lvl-3));
    ...
    for(anzahl_Taster > 0; anzahl_Taster--;)
    {
      ...
      while(Taster_Schleife == false) // leuft bis eine Taste gedrückt wurde
      {
          ...
          if(Taster_In)
          {
              ...
              {
                int x; //LED die beim richtigen Taster leuchtet
                switch (Taster_In) 
                {
                    case 'D':
                        x = 0;
              ...
              }else
              {
                ...
                pixels.fill(pixels.Color(255, 0, 0), 0, 16);
                pixels.show();
                delay(400);
                pixels.clear();
                pixels.show();
